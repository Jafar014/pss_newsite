...
const videos = [
    ...
    { id: 'qOeLUzG5Dw4', title: 'Post Match Press Conference Pasca Final' },
];
...
export default function HomeVideo() {
    ...
    const below = videos.slice(1);

    return (
        <div className="relative w-full min-h-[550px] sm:min-h-[700px] lg:h-[920px]">
            ...
            <div className="absolute inset-0 bg-[#0f7a4a] bg-opacity-50 py-6 sm:py-10 md:py-14 px-4 sm:px-5 md:px-6 overflow-y-auto">
                <div className="w-full h-full flex flex-col gap-3 sm:gap-4">
                    ...
                    <div className="w-full max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-4 sm:gap-6 flex-1 min-h-0">
                        ...
                        <div className="w-full lg:flex-1 rounded-2xl overflow-hidden bg-black">
                            <div className="relative w-full aspect-video">
                                {isPlaying ? (
                                    ...
                                    <div
                                        ...
                                        style={{ backgroundImage: `url('https://img.youtube.com/vi/${headline.id}/maxresdefault.jpg')` }}
                    ...
                    {/* Divider */}
                    <div className="w-full max-w-[1400px] mx-auto shrink-0 h-px bg-white/30 hidden sm:block" />

                    {/* Grid */}
                    <div className="w-full max-w-[1400px] mx-auto shrink-0 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                        {below.map((video, idx) => (
                            <div
                                key={video.id}
                                className="flex items-center gap-3 sm:gap-4 cursor-pointer group min-w-0"
                                onClick={() => { setActiveIdx(idx + 1); setIsPlaying(true); }}
                            >
                                <div className="relative w-28 sm:w-36 md:w-44 lg:w-52 aspect-video rounded-lg overflow-hidden shrink-0 bg-black">
                                    <div
                                    ...
                                    <div className="absolute inset-0 bg-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                                        <div className="w-10 h-10 rounded-full bg-white/90 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
                                            <svg className="w-5 h-5 text-[#0f7a4a] ml-0.5" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M8 5v14l11-7z" />
                                            </svg>
                                        </div>
                                    </div>
